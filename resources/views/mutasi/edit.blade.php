...
<div class="container mx-auto p-4">
    <h1 class="text-2xl font-bold mb-4">Edit Mutasi</h1>

    <form action="{{ route('mutasi.update', $mutasi->id) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="produk_id" id="produk_id" class="border p-2 w-full">
                @foreach($produks as $produk)
                    <option value="{{ $produk->id }}" {{ $mutasi->produk_id == $produk->id ? 'selected' : '' }}>{{ $produk->nama_produk }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="lokasi_asal_id" id="lokasi_asal_id" class="border p-2 w-full">
                @foreach($lokasis as $lokasi)
                    <option value="{{ $lokasi->id }}" {{ $mutasi->lokasi_asal_id == $lokasi->id ? 'selected' : '' }}>{{ $lokasi->nama_lokasi }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="lokasi_tujuan_id" id="lokasi_tujuan_id" class="border p-2 w-full">
                @foreach($lokasis as $lokasi)
                    <option value="{{ $lokasi->id }}" {{ $mutasi->lokasi_tujuan_id == $lokasi->id ? 'selected' : '' }}>{{ $lokasi->nama_lokasi }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="number" name="jumlah" id="jumlah" class="border p-2 w-full" value="{{ old('jumlah', $mutasi->jumlah) }}">
            @error('jumlah') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>

        <div>
            <label for="tanggal" class="block font-medium">Tanggal</label>
            <input type="date" name="tanggal" id="tanggal" class="border p-2 w-full" value="{{ old('tanggal', $mutasi->tanggal) }}">
